...
if (isset($_SESSION['logged_in'])) {
    ...
    if($sector_type_id == 2){
        ...
        }else if($report_id === '4' || $report_id === '5'){ 
        ...
        if($debug){
            ...
            if($county_id === "all"){
                ...
                $sql = "select indicator_id, age_range, gender, counties.county_name, sum(aggregate) as aggregate from health_aggregates join counties on counties.county_id=health_aggregates.county_id " .
                    " where indicator_id in (" . $indicator_query_N . ") and date between '" .
                    $fromdate_new . "' and '" . $todate_new . "' group by indicator_id, gender, age_range, health_aggregates.county_id";
            }
        if($debug){
                   print $sql . "<br>";
            }
            ...
            if($county_id === "across"){
                //create data with all counties for cross county report
                foreach ( $counties as $urows ) {
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['MALE_LESS18_N_4'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['MALE_LESS18_N_5'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['MALE_LESS18_N_39'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['MALE_LESS18_D'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['MALE_MORE18_N_4'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['MALE_MORE18_N_5'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['MALE_MORE18_N_39'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['MALE_MORE18_D'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['FEMALE_LESS18_N_4'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['FEMALE_LESS18_N_5'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['FEMALE_LESS18_N_6'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['FEMALE_LESS18_N_39'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['FEMALE_LESS18_D'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['FEMALE_MORE18_N_4'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['FEMALE_MORE18_N_5'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['FEMALE_MORE18_N_6'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['FEMALE_MORE18_N_39'] = 0;
                    $data_hash_across_6[str_replace("'", "", $urows ['1'])]['FEMALE_MORE18_D'] = 0;
                }
                foreach ( $data_all_D as $urows3 ) {
                    array_push($data, $urows3); //do not show "No data" message
                    if(strtoupper($urows3 ['gender']) === "MALE"){
                        if($urows3 ['age_range'] === "0-11" || $urows3 ['age_range'] === "12-17"){
                            $data_hash_across_6[str_replace("'", "", $urows3['county_name'])]['MALE_LESS18_D'] += $urows3['aggregate'];
                        }else{
                            $data_hash_across_6[str_replace("'", "", $urows3['county_name'])]['MALE_MORE18_D'] += $urows3['aggregate'];
                        }
                    }else if(strtoupper($urows3 ['gender']) === "FEMALE"){
                        if($urows3 ['age_range'] === "0-11" || $urows3 ['age_range'] === "12-17"){
                            $data_hash_across_6[str_replace("'", "", $urows3['county_name'])]['FEMALE_LESS18_D'] += $urows3['aggregate'];
                        }else{
                            $data_hash_across_6[str_replace("'", "", $urows3['county_name'])]['FEMALE_MORE18_D'] += $urows3['aggregate'];
                        }
                    }
                    foreach ( $data_all_N as $urows4 ){
                        if($urows4 ['county_name'] === $urows3['county_name'] && 
                            ...
                            strtoupper($urows4 ['gender']) === strtoupper($urows3['gender'])){
                                if(strtoupper($urows4 ['gender']) === "MALE"){
                                    if($urows4 ['age_range'] === "0-11" || $urows4 ['age_range'] === "12-17"){
                                        $data_hash_across_6[str_replace("'", "", $urows4['county_name'])]['MALE_LESS18_N_'.$urows4 ['indicator_id']] += $urows4['aggregate'];
                                    }else{
                                        $data_hash_across_6[str_replace("'", "", $urows4['county_name'])]['MALE_MORE18_N_'.$urows4 ['indicator_id']] += $urows4['aggregate'];
                                    }
                                }else if(strtoupper($urows4 ['gender']) === "FEMALE"){
                                    if($urows4 ['age_range'] === "0-11" || $urows4 ['age_range'] === "12-17"){
                                        $data_hash_across_6[str_replace("'", "", $urows4['county_name'])]['FEMALE_LESS18_N_'.$urows4 ['indicator_id']] += $urows4['aggregate'];
                                    }else{
                                        $data_hash_across_6[str_replace("'", "", $urows4['county_name'])]['FEMALE_MORE18_N_'.$urows4 ['indicator_id']] += $urows4['aggregate'];
                                    }
                               }
                              unset($urows4);
                        }
...
function pdf_across_report6(){
	var doc = new jsPDF('l', 'pt', 'a4', false);
		...
		try {
	     doc.addImage(image1[0], 'png', 40, 100, 500, 250); 
	      }catch (e) {
	...
	if(image2.length > 0){
		//doc.addPage();
		try {
		    doc.addImage(image2[0], 'png', 40, 100, 500, 250);
		}catch (e) {
	        ...
	        $("#chart_title").html("<h2 id='chart_title_1' class='page-title-section'><?php echo $report_name . "</h2><h2 id='chart_title_2' class='page-title-section'>" . $date_title . "</h2>";?>");
		      ...
		      //draw table
	       	 var data_table = new google.visualization.DataTable();
		       	...
		       	data_table.addRows([
	        	<?php foreach ( $data_hash_across_6 as $key=>$urows) {
	        	    $male_total = $urows['MALE_LESS18_N_4']+$urows['MALE_LESS18_N_5']+$urows['MALE_LESS18_N_39'];
	        	    $male = 0;
	        	    if($urows['MALE_LESS18_D'] >0){
	        	        $male = round(($male_total/$urows['MALE_LESS18_D'])*100,2);
	        	    }
	        	    $female_total = $urows['FEMALE_LESS18_N_4']+$urows['FEMALE_LESS18_N_5']+$urows['FEMALE_LESS18_N_6']+$urows['FEMALE_LESS18_N_39'];
	        	    $female = 0;
...
echo "[{v:'" . $key . "'}, {v:" . $urows['MALE_LESS18_D'] . "}, {v:" . 
    ...
    $female_total . "},{v:" . $female . "}],";
	         ...
	         table.draw(data_table, {allowHtml: true, showRowNumber: false, width: '1000px', height: '100%'});
	         ...
	         data_chart = new google.visualization.DataTable();
	        	...
	        	data_chart.addRows([
	        	<?php 
	        	foreach ( $data_hash_across_6 as $key=>$urows ) {
	        	    $male = 0;
	        	    $male_total = $urows['MALE_LESS18_N_4']+$urows['MALE_LESS18_N_5']+$urows['MALE_LESS18_N_39'];
	        	    if($urows['MALE_LESS18_D'] >0){
	        	        $male = round(($male_total/$urows['MALE_LESS18_D'])*100,2);
			...
			chart.draw(data_chart, options);
			 ...
			 //draw table 2
	       	 var data_table2 = new google.visualization.DataTable();
		       	...
		       	data_table2.addRows([
	        	<?php foreach ( $data_hash_across_6 as $key=>$urows) {
	        	    $male_total = $urows['MALE_MORE18_N_4']+$urows['MALE_MORE18_N_5']+$urows['MALE_MORE18_N_39'];
	        	    $male = 0;
	        	    if($urows['MALE_MORE18_D'] >0){
	        	        $male = round(($male_total/$urows['MALE_MORE18_D'])*100,2);
	        	    }
	        	    $female_total = $urows['FEMALE_MORE18_N_4']+$urows['FEMALE_MORE18_N_5']+$urows['FEMALE_MORE18_N_6']+$urows['FEMALE_MORE18_N_39'];
	        	    $female = 0;
...
{v:" . $female . "}],";
        ...
        $male = 0;
	        	    ...
	        	    $female = 0;
	        	    $female_total = $urows['FEMALE_MORE18_N_4']+$urows['FEMALE_MORE18_N_5']+$urows['FEMALE_MORE18_N_6']+$urows['MALE_FEMORE18_N_39'];
	        	    if($urows['FEMALE_MORE18_D'] >0){
	        	        $female = round(($female_total/$urows['FEMALE_MORE18_D'])*100,2);
	        	    }
          echo "[ '" . $key . "', {v:" . $male . "}, {v:" . $female . "}],";
